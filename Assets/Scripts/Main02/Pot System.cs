...
{
    [Header("RectTransform")] public RectTransform Sugar;
    public RectTransform Water;

    [Header("GameObject")] public GameObject Fires;
    public GameObject FruitStick;

    [Header("애니메이션")] public AnimationSequencerController MeltingSugarSequence;
    [SerializeField] private Animator stickAnimator; // 스틱 애니메이터
    public float PourSpeed = 0.1f; // 채워지는 속도

    [Header("결과 배경")] [SerializeField] private GameObject[] resultBackgrounds;

    [Header("가스레인지 사운드"), SerializeField] private AudioSource gasRangeAudioSource;

    [Header("안내 텍스트")] [SerializeField] private TextMeshProUGUI InfoText;
    public string[] InfoTexts;

    public bool Pouring { get; set; }

    private float _pourAmount;
    ...
    private void Start()
    {
        ...
        // 설탕과 물을 스왑하는 처리
        StartCoroutine(SwapSugarAndWaterTask());

        // 모두 채워지면 Water Pot를 비활성화 처리 및 애니메이션 적용
        StartCoroutine(TriggerLeverAnimationTask());
    }
    ...
    /// </summary>
    /// <returns></returns>
    private IEnumerator SwapSugarAndWaterTask()
    {
        yield return new WaitUntil(() => _pourAmount >= BestSugarAmount);
    ...
    /// </summary>
    /// <returns></returns>
    private IEnumerator TriggerLeverAnimationTask()
    {
        yield return new WaitUntil(() => _pourAmount >= 1.0f);

        // 물을 비활성화하고 레버를 활성화 합니다.
        _waterPot.Interaction = false;
        _lever.Interaction = true;

        // _waterPot가 Interaction이 False되면서 Disalbe처리 함수가 호출이 원활하게 되도록 0.01초 정도 기다립니다.
    ...
    private void Update()
    {
        // 부어지는 양을 계산합니다.
        if (Pouring)
            _pourAmount += PourSpeed * Time.deltaTime;

        // 0~1 사이의 값으로 제한
        _pourAmount = Mathf.Clamp01(_pourAmount);

        // 설탕은 BestSugarAmount(60%) 이상까지만 처리되고, 그 다음 물을 채워준다.
        if (_pourAmount <= BestSugarAmount)
        {
            var sugarY = Mathf.Lerp(PotFillRange.x, PotFillRange.y, _pourAmount);
            Sugar.anchoredPosition = new Vector2(0, sugarY);
        }
        else
        {
            var waterY = Mathf.Lerp(PotFillRange.x, PotFillRange.y, _pourAmount);
